...
prev_axes = {"left_stick": {"x": 0, "y": 0}, "right_stick": {"x": 0, "y": 0}}
                ...
                while "}" in buffer:
                    try:
                        ...
                        if parsed_data.get("type") == "axis":
                            ...
                            if (l_stick_x != prev_axes["left_stick"]["x"]) or (
                                l_stick_y != prev_axes["left_stick"]["y"]
                            ):
                                controller.set_value("AxisLx", l_stick_x)
                                controller.set_value("AxisLy", l_stick_y)

                            if (r_stick_x != prev_axes["right_stick"]["x"]) or (
                                r_stick_y != prev_axes["right_stick"]["y"]
                            ):
